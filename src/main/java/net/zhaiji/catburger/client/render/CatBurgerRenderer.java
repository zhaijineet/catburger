...
public class CatBurgerRenderer implements ICurioRenderer {
    public static double getFloatSpeed(int tickCount, float partialTicks) {
        return CatBurgerClientConfig.floatDistance / 2 * Math.sin((tickCount + partialTicks) * Mth.HALF_PI / CatBurgerClientConfig.time);
    }
    ...
    public <S extends LivingEntityRenderState, M extends EntityModel<? super S>> void render(
        ...
        // 使用新 API 渲染 item：通过 ItemModelResolver 解析模型，再通过 ItemStackRenderState 提交
        ItemStackRenderState itemRenderState = new ItemStackRenderState();
        context.getItemModelResolver().updateForLiving(
                itemRenderState, stack, ItemDisplayContext.HEAD, entity
        );
        if (!itemRenderState.isEmpty()) {
            itemRenderState.submit(
                    poseStack, submitNodeCollector,
                    packedLight, OverlayTexture.NO_OVERLAY, renderState.outlineColor
            );
        }
